...
前言：由于采用自动化或协议来注册codex时，会出现手机验证码，特意开发了一个可以支持屏幕悬停的邮件验证码获取工具，目前该工具仅支持cloudmail自建邮箱。

功能：自动获取cpa的codex-oauth登入链接，获取后自动复制到粘贴板，生成设定好的邮箱和密码，只需要设置邮箱的域名和前缀(所以给一个值段，如test，则生成的邮箱为test00001@cloudmail.com，到99999)，并自动获取验证码进入粘贴板（注意：到验证码界面时需要点击重新获取邮箱才会收到验证码)，注册完成后会自动回调到cpa中，并自动刷新账号数量。
